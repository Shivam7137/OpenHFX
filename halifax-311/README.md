...
export ANTHROPIC_API_KEY=...        # без ключа работает на фикстуре и честно это показывает
python3 demo/server.py              # http://127.0.0.1:8768/
```

Экран подачи заявки в их дизайн-токенах, с голосовым вводом (Web Speech API,
Chrome/Safari; ввод текстом работает везде). Показывает то, что в их макете
захардкожено: категорию, приоритет, выбранную форму, уверенность с порогами
и заполненные поля с реальными ключами Halifax.

Сервер отдаёт весь репозиторий, поэтому их `reference.html` остаётся доступен
по своему обычному адресу и не тронут.
